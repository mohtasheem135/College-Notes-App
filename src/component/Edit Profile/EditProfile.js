...
const EditProfile = () => {
  ...
  const { name, pemail, cemail, roll, semester, department, college, passCode } = initialState;
  ...
  const handelSubmit = (e) => {
    ...
    fireDB.database().ref().child(`Profile`).child(`${initialState.roll}`).set(initialState, (err) => {
      if (err) {
        console.log(err);
      } else {
